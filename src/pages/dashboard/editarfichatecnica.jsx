...
const fetchExistingInsumos = async () => {
  try {
    const response = await axios.get("http://localhost:3005/api/insumos"); // Asegúrate de que esta URL es correcta
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(`http://localhost:3005/api/fichastecnicas/${selectedFicha.id_ficha}`, fichaToSave);
      Toast.fire({
        icon: 'success',
