...
export function CategoriaInsumos() {
  ...
  const fetchCategorias = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/categorias_insumo");
  ...
  const handleDelete = async (categoria) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/categorias_insumo/${categoria.id_categoria}`);
        fetchCategorias();
 ...
 const isDuplicate = categorias.some((categoria) =>
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://localhost:3005/api/categorias_insumo/${selectedCategoria.id_categoria}`, selectedCategoria);
        setOpen(false);
        fetchCategorias();
        const Toast = Swal.mixin({
          toast: true,
        ...
        await axios.post("http://localhost:3005/api/categorias_insumo", selectedCategoria);
        fetchCategorias();
        setOpen(false);
        const Toast = Swal.mixin({
          toast: true,
    ...
    if (result.isConfirmed) {
      try {
        if (categoria.estado) { 
          ...
          const response = await axios.get(`http://localhost:3005/api/insumos?categoria_id=${categoria.id_categoria}`);
          const insumosAsociados = response.data;         
        }
        await axios.patch(`http://localhost:3005/api/categorias_insumo/${categoria.id_categoria}/estado`, { estado: !categoria.estado });
        fetchCategorias();
        Toast.fire({
          icon: 'success',
