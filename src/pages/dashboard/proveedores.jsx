...
export function Proveedores() {
  ...
  const handleDelete = async (proveedor) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/proveedores/${proveedor.id_proveedor}`);
  ...
  const handleSave = async () => {
    ...
    if (validateForm()) {
      try {
        if (editMode) {
          await axios.put(`http://localhost:3005/api/proveedores/${selectedProveedor.id_proveedor}`, selectedProveedor);
          Toast.fire({
            icon: 'success',
            title: 'El proveedor ha sido actualizado correctamente.'
          });
        } else {
          await axios.post("http://localhost:3005/api/proveedores", selectedProveedor);
          Toast.fire({
            icon: 'success',
  ...
  const toggleActivo = async (id_proveedor, estado) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.patch(`http://localhost:3005/api/proveedores/${id_proveedor}/estado`, { estado: !estado });
        fetchProveedores(); 
        Toast.fire({
          icon: 'success',
