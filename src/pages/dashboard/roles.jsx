...
export function Roles() {
  ...
  const fetchRoles = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/roles");
  ...
  const fetchPermisos = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/permisos");
  ...
  const handleDelete = async (role) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/roles/${role.id_rol}`);
        fetchRoles(); 
        Toast.fire({
          icon: "Rol Eliminado",
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://localhost:3005/api/roles/${selectedRole.id_rol}`, {
        ...
        await axios.post("http://localhost:3005/api/roles", {
          nombre: selectedRole.nombre,
          permisos: selectedRole.permisosRol,
          estado: selectedRole.estado,
        });
  ...
  const toggleActivo = async (id_rol, estado) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.patch(`http://localhost:3005/api/roles/${id_rol}/estado`, { estado: !estado });
        fetchRoles();
        Toast.fire({
          icon: 'success',
