  ...
  if (exists) {
    ...
    try {
      if (selectedCliente.id_cliente) {
        await axios.put(`http://localhost:3005/api/clientes/${selectedCliente.id_cliente}`, selectedCliente);
        Toast.fire({
          icon: 'success',
          title: 'El cliente ha sido actualizado correctamente.'
        });
      } else {
        await axios.post("http://localhost:3005/api/clientes", selectedCliente);
        Toast.fire({
          icon: 'success',
