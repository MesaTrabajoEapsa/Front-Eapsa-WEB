...
export function FichasTecnicas() {
  ...
  const fetchFichas = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/fichastecnicas");
  ...
  const handleDelete = async (ficha) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/fichastecnicas/${ficha.id_ficha}`);
        fetchFichas();
        Toast.fire({
          icon: "success",
  ...
  const toggleActivo = async (id_ficha, estado) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.patch(`http://localhost:3005/api/fichastecnicas/${id_ficha}/estado`, { estado: !estado });
        fetchFichas();
        Toast.fire({
          icon: 'success',
