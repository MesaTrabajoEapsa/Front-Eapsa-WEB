...
export function EditarProduccion({ open, handleEditProductionOpen, orden }) {
  ...
  const fetchVentas = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/ventas");
  ...
  const fetchTotalProductosEnProduccion = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/ordenesproduccion?estado=2");
  ...
  const fetchVentasAsociadas = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/ordenesproduccion/todas_ventas_asociadas");
  ...
  const loadOrderDetails = async () => {
    try {
      const response = await axios.get(`http://localhost:3005/api/ordenesproduccion/${orden.id_orden}/ventas_asociadas`);
      const ventasAsociadas = response.data.map(venta => venta.numero_venta);
  ...
  const handleUpdateProductionSave = async () => {
    ...
    try {
      await axios.put(`http://localhost:3005/api/ordenesproduccion/${orden.id_orden}`, updatedOrder);
      Swal.fire({
        icon: "success",
