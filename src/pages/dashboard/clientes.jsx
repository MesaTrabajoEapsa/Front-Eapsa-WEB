...
export function Clientes() {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/clientes/${id}`);
        fetchClientes();
        Toast.fire({
          icon: 'success',
  ...
  const handleToggleEstado = async (cliente) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.patch(`http://localhost:3005/api/clientes/${cliente.id_cliente}/estado`, {
  ...
  const verificarAsociaciones = async (id_cliente) => {
    try {
      const response = await axios.get(`http://localhost:3005/api/clientes/${id_cliente}/verificar-asociaciones`);
