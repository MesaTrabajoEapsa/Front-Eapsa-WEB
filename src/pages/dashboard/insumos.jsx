...
export function Insumos() {
  ...
  const fetchInsumos = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/insumos");
  ...
  const handleDelete = async (insumo) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/insumos/${insumo.id_insumo}`);
        fetchInsumos();
  ...
  const handleSave = async () => {
    try {
      ...
      if (editMode) {
        await axios.put(`http://localhost:3005/api/insumos/${selectedInsumo.id_insumo}`, selectedInsumo);
        setOpen(false);
        ...
        await axios.post("http://localhost:3005/api/insumos", selectedInsumo);
        fetchInsumos();
        setOpen(false);
        const Toast = Swal.mixin({
          toast: true,
    ...
    if (result.isConfirmed) {
      try {
        if (!estado) { // Solo verificamos si intentamos desactivar el insumo   
          const fichasResponse = await axios.get(`http://localhost:3005/api/fichastecnicas`);
        ...
        await axios.patch(`hhttp://localhost:3005/api/insumos/${insumo.id_insumo}/estado`, {
          estado: estado
        });
        ...
        <DialogBody className="space-y-2">
          <div className="space-y-1">
            ...
            <Typography className="text-sm">
              {selectedInsumo.categoriaInsumo?.nombre || 'N/A'} {/**/}
            </Typography>
