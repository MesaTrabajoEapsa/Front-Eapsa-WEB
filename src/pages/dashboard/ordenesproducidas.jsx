...
export function OrdenesProducidas({handleBackToMain}) {
  ...
  const fetchOrdenesProducidas = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/ordenesproduccion/producidas");
  ...
  const handleCompleteOrder = async (idOrden) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.put(`http://localhost:3005/api/ordenesproduccion/${idOrden}/estado`, {
          estado: 'completado',
        });
        Toast.fire({
          icon: 'success',
