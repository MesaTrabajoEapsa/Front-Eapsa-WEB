...
export function ProductoTerminado() {
  ...
  const fetchProductosActivos = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/productos/activos");
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://localhost:3005/api/productos/${selectedProducto.id_producto}`, selectedProducto);
        Toast.fire({
          icon: 'success',
          title: 'El producto ha sido actualizado correctamente.'
        });
      } else {
        await axios.post("http://localhost:3005/api/productos", selectedProducto);
        Toast.fire({
          icon: 'success',
  ...
  const handleDelete = async (producto) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:3005/api/productos/${producto.id_producto}`);
        fetchProductos();
        Toast.fire({
          icon: 'success',
  ...
  const toggleActivo = async (id_producto, estado) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const ordenesResponse = await axios.get(`http://localhost:3005/api/ordenesProduccion`);
        ...
        const ventasResponse = await axios.get(`http://localhost:3005/api/ventas`);
        ...
        await axios.patch(`http://localhost:3005/api/productos/${id_producto}/estado`, { estado: !estado });
        fetchProductos();
        fetchProductosActivos();
        const Toast = Swal.mixin({
          toast: true,
