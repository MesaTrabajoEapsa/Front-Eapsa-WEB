...
export function OrdenesInactivas({handleBackToMain}) {
  ...
  const fetchOrdenesInactivas = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/ordenesproduccion/inactivas");
  ...
  const handleActivateOrder = async (idOrden) => {
    try {
      await axios.patch(`http://localhost:3005/api/ordenesproduccion/${idOrden}/estado`, {
        activo: true
      });
      Toast.fire({
        icon: 'success',
