...
export function OrdenesProduccion() {
  ...
  const fetchOrdenes = async () => {
    try {
      const response = await axios.get("http://localhost:3005/api/ordenesproduccion");
  ...
  const handleProducir = async (idOrden) => {
    try {
      await axios.post(`http://localhost:3005/api/ordenesproduccion/${idOrden}/producir`);
      Toast.fire({
        icon: 'success',
  ...
  const handleConfirmAnulacion = async () => {
    ...
    try {
      await axios.patch(`http://localhost:3005/api/ordenesproduccion/${ordenToAnular.id_orden}/estado`, {
